...
Mat* MatCreate(Mat* mat, int row, int col)
{
	...
	mat->element = (float**)mymalloc(SRAMEX, row * sizeof(float*));
	if(mat->element == NULL){
		printf("mat create fail!\n");
		return NULL;
	}
	for(i = 0 ; i < row ; i++){
		mat->element[i] = (float*)mymalloc(SRAMEX, col * sizeof(float));	
...
float MatDet(Mat* mat)
{
	...
	list = (int*)mymalloc(SRAMEX, sizeof(int)*mat->col);
	if(list == NULL){
...
void MatPrint(Mat *mat, char* mat_name) {
	int i, j;
	for(i = 0; i < mat->row; i++) {
		for(j = 0; j < mat->col; j++) {
			printf("%s[%d][%d] = %f \t", mat_name, i, j, mat->element[i][j]);
		}
		printf("\r\n");
	}
}
